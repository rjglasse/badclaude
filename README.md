...
- It **can't touch the world**. It can describe a Go program beautifully,
...
1. Install Go 1.21 or newer (`go version` to check).
...
**Rather work in another language?** This is the Go branch. BadClaude also
comes in Java, Python and Rust, one branch each: `git checkout main` (Java),
`git checkout python` or `git checkout rust`. Same course, same steps,
different syntax.
...
| File | What it does |
| --- | --- |
| `main.go` | Wires everything together and starts the loop |
| `harness.go` | The chat loop: read → send → print |
| `llm_client.go` | Sends messages to the API, returns the reply |
| `message.go` | One chat message (role + content) |
| `memory.go` | Interface: what the harness remembers |
| `no_memory.go` | The default memory: none at all |
| `tool.go` | Interface: things the model can ask the harness to do |
| `interaction_log.go` | Logs every session to `logs/*.jsonl` |
| `config.go` | Reads `config.properties` |
| `go.mod` | Tells Go this folder is the `badclaude` module |

No dependencies: everything comes from Go's standard library (`net/http`
talks to the API, `encoding/json` reads and writes JSON). `go run .` is all
you need, and the run scripts do that for you.
...
4. **Change anything, and keep it tidy.** The starter code is yours: split
   `Run()` into functions, add types, rename, move and delete. The seams we
