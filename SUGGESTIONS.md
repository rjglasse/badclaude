...
with what you learned in CS1/CS2: classes, abstract base classes, lists,
dictionaries, loops, recursion, and file I/O.
...
**The fix:** Write a `ConversationMemory` class that subclasses `Memory`,
...
*Concepts: abstract base classes and subclasses, lists, queues.*
...
**The fix:** Experiment with `SYSTEM_PROMPT` in `harness.py`. Give it a
...
*Concepts: none new — this one teaches you about the model, not about Python.*
...
1. Subclass the `Tool` base class. Good first tools: a **calculator**, a
...
*Concepts: abstract base classes and polymorphism, string parsing, protocols.*
...
A raw Python exception like
`ValueError: invalid literal for int() with base 10: '6.'` teaches it
nothing, and it will just repeat itself until your lap limit stops it.
Say what was wrong and what right looks like:
...
**The problem:** BadClaude can describe a Python program but can't build one.
...
- `check`: run `python -m py_compile` on a file (or `python -m unittest` on
  the workspace to run its tests) using the `subprocess` module, capture
  the output, and return it — **including errors**, because feeding errors
  back is exactly how the model fixes its own bugs.
- `run`: run the program with `python` and return what it printed.
